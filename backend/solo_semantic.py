...
class SoloSemanticBuffer:
    ...
    Параметры таймеров (flush_after_sec / hard_flush_sec) задаются при создании
    в main.py (runtime: 7.0 / 12.0). Значения по умолчанию ниже — запасные.
    """

    # Аварийные пороги. В нормальной работе НЕ должны срабатывать —
    # окно flush_after_sec (runtime: 7с, см. main.py) вычистит carry задолго
    # до этих лимитов. Значения по умолчанию у конструктора (5/8) переопределяются
    # в main.py значениями 7/12.
    MAX_CARRY_WORDS = 70       # ~30-40 секунд речи
    MAX_CARRY_AGE_SEC = 20.0   # запас сверх нескольких окон flush
