...
ПРОБЛЕМА (production-аудит): в текущем Railway deployment Gmail SMTP оказался
недоступен: `[Errno 101] Network is unreachable`. Из-за этого verification
email просто не отправлялся, хотя пользователь видел «успех». (Это наблюдение
по конкретному деплою, а не доказанное общее правило Railway.)
